...
class driving_mode_dataloader(tf.keras.utils.Sequence):
    def __init__(self, dataset_path, max_km, driver, data, pre_sec, batch_size=32, valid_ratio=0.2, image_size='large', no_response='ignore', bio_columns = ['EDA', 'BVP']):
        ...
        if self.data == 'front_image':
            ...
            if odd in self.x_dict.keys():
                ...
                if self.data == 'bio':
                    ...
                    x = self.x_dict[odd][st_index:end_index]
                    # x = np.reshape(x, (self.per_sec[self.data]*self.pre_sec, 1, 3))
                if self.data == 'audio':
    ...
    def get_odd_indices(self, odd):
        ...
        if self.data == 'audio':
            ...
            for odd, tmp_time, status in label_time_indices:
                tmp_index = int(self.per_sec[self.data]*(tmp_time - audio_st_time))
    ...
    def get_test_data(self, test_odd):
        ...
        odd_test_list = [(odd, (st_index, end_index), status) for (odd, (st_index, end_index), status) in self.test_indices if odd == test_odd]
        if self.data in ['side_image', 'front_image']:
            color_list = []
            ir_list = []
            y_list = []
            for i, (odd, (st_index, end_index), status) in enumerate(odd_test_list):
                color_x, ir_x = self.get_data(odd, st_index, end_index, status)
...
def main():
    ...
    dataloader = driving_mode_dataloader(
        ...
        pre_sec=2,  # pre second from self-reporting
    ...
    for epoch in range(epochs):
        ...
        for i, (x, y) in enumerate(train_loader):
            ...
            # audio_path = '/home/euiseokjeong/Desktop/driving_mode_2nd_algo/tmp_audio'
            # for j, audio in enumerate(x):
            #     print(os.path.join(audio_path, f'Euiseok_train_{i}_{j}.wav'))
            #     write(os.path.join(audio_path, f'Euiseok_train_{i}_{j}.wav'), 44100, audio)
