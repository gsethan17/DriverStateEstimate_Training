...
class driving_mode_dataloader(tf.keras.utils.Sequence):
    def __init__(self, dataset_path, max_km, driver, data, pre_sec, get_serial, batch_size=32, valid_ratio=0.2, get_odd=False, image_size='large', no_response='ignore', bio_columns = ['EDA', 'BVP'],audio_offset=3):
        self.dms_dataset_path = dataset_path
        self.batch_size = batch_size
        self.driver = driver
        self.audio_offset = audio_offset
        self.bio_columns = bio_columns
        self.get_serial = get_serial
        self.data = data
        self.get_odd = get_odd
        self.image_size = image_size
        self.per_sec, self.min_max_dict = self.get_per_sec()
    ...
    def get_label_weights(self):
        ...
        train_valid_list = self.train_data_list + self.valid_data_list
        for data in train_valid_list:
            label_df = pd.read_csv(glob(os.path.join(self.driver_path, data, 'HMI', '*.csv'))[0])
            for i in range(len(label_df)):
                tmp_status = int(label_df['status'].iloc[i])
                result_list[tmp_status] += 1
        result_weight = [sum(result_list) / (len(result_list) * x) for x in result_list[1:]]
        return (result_list, result_weight)
        ...
        if self.data == 'front_image':
            ...
            if odd in self.x_dict.keys():
                ...
                if self.data == 'bio':
                    ...
                    self.x_dict[odd] = np.array(bio_df.drop(['timestamp'], axis=1)).reshape((len(bio_df), 1, len(self.bio_columns)))
                    x = self.x_dict[odd][st_index:end_index]
                if self.data == 'audio':
                    hz, audio_np = read(glob(os.path.join(self.driver_path, odd, 'audio', '*.wav'))[0])
    ...
    def get_odd_indices(self, odd):
        ...
        result = []
        # add serial number
        label_time_indices = [(odd, label_df['time'].iloc[i], label_df['status'].iloc[i], f'{str(self.driver)[0]}_{odd}_{i}') for i in range(len(label_df))]

        if self.data == 'audio':
            audio_time_df = pd.read_csv(glob(os.path.join(self.driver_path, odd, 'audio', '*.csv'))[0])
            audio_st_time = audio_time_df['timestamp'].iloc[0]
            for odd, tmp_time, status, serial in label_time_indices:
                tmp_index = int(self.per_sec[self.data]*(tmp_time - audio_st_time) + self.audio_offset*self.per_sec[self.data])
                result.append((odd, (tmp_index - int(self.pre_sec * self.per_sec[self.data]), tmp_index), status, serial))
        ...
        result = [(odd, (st_index, end_index), status, serial) for (odd, (st_index, end_index), status, serial) in result if st_index >= 0]
    ...
    def get_bio_df(self, odd):
        ...
        for bio_data in self.bio_columns:
            ...
            if bio_data in ['EDA','BVP']:
                ...
                """
                remove outlier
                arguments: df, bio_data in ['EDA', 'BVP'. 'HR']
                """
                def remove_outlier(x):
                    upper_bound = self.min_max_dict[self.driver][bio_data]['max']
                    lower_bound = self.min_max_dict[self.driver][bio_data]['min']
                    if x >= upper_bound:
                        x = upper_bound
                    elif x <= lower_bound:
                        x = lower_bound
                    return (x-lower_bound)/(upper_bound-lower_bound)
                df[bio_data] = df[bio_data].apply(remove_outlier)
    ...
    def get_data_list(self, max_km, valid_ratio):
        ...
        for odd in self.total_data_list:
            max_train_km += self.get_data_km(odd)
        min_train_km = 0
        for odd in self.total_data_list[:2]:
            min_train_km += self.get_data_km(odd)
        for odd in self.total_data_list:
        ...
        if len(train_data_list) == 0:
            raise ValueError(f'The length of train_data_list is {len(train_data_list)}. You should set maximum minimum train km to {min_train_km+1}')

        if len(test_data_list) == 0:
            raise ValueError(
                f'The length of test_data_list is {len(test_data_list)}. You can set maximum maximum train km up to {max_train_km}')
    ...
    def shuffle_data(self):
        tmp_dict = {}
        for odd, indices, status, serial in self.train_indices:
        ...
        tmp_dict = {}
        for odd, indices, status, serial in self.valid_indices:
            if odd in tmp_dict.keys():
                tmp_dict[odd].append((odd, indices, status))
            else:
                tmp_dict[odd] = [(odd, indices, status)]
    ...
    def get_train_data(self):
        if self.data in ['side_image', 'front_image']:
            ...
            odd_list = []
            serial_list=[]
            for i, (odd, (st_index, end_index), status, serial) in enumerate(self.train_indices):
                ...
                if (i + 1) % self.batch_size == 0:
                    if len(color_list) != 0:
                        ...
                        if self.get_serial:
                            yield (np.stack(color_list), np.stack(ir_list)), np.stack(y_list), np.stack(serial_list)
                        elif self.get_odd:
                            yield (np.stack(color_list), np.stack(ir_list)), np.stack(y_list), np.stack(odd_list)
                        else:
            ...
            serial_list = []
            for i, (odd, (st_index, end_index), status, serial) in enumerate(self.train_indices):
                x = self.get_data(odd, st_index, end_index, status)
                if len(x) == self.per_sec[self.data]*self.pre_sec:
                    x_list.append(x)
                    y_list.append(status)
                    odd_list.append(odd)
                    serial_list.append(serial)
                    if len(x_list) == self.batch_size:
                        if self.get_serial:
                            yield np.stack(x_list),  np.stack(y_list), np.stack(serial_list)
                        elif self.get_odd:
                            yield np.stack(x_list), np.stack(y_list), np.stack(odd_list)
                        else:
    ...
    def get_valid_data(self):
        if self.data in ['side_image', 'front_image']:
            ...
            odd_list = []
            serial_list = []
            for i, (odd, (st_index, end_index), status, serial) in enumerate(self.valid_indices):
                color_x, ir_x = self.get_data(odd, st_index, end_index, status)
    ...
    def get_test_num(self):
        ...
        for i in self.test_data_list:
            test_km_list.append(self.get_data_km(i))
        return len(self.test_data_list), test_km_list, self.test_data_list

    def get_test_data(self, test_odd):
        print(f"[INFO] {test_odd} Test data: {self.get_data_km(test_odd)}km")
        odd_test_list = [(odd, (st_index, end_index), status, serial) for (odd, (st_index, end_index), status, serial) in self.test_indices if odd == test_odd]
        if self.data in ['side_image', 'front_image']:
            color_list = []
            ir_list = []
            y_list = []
            odd_list = []
            serial_list = []
            for i, (odd, (st_index, end_index), status, serial) in enumerate(odd_test_list):
                color_x, ir_x = self.get_data(odd, st_index, end_index, status)
    ...
    def get_per_sec(self):
        ...
        min_max_dict = {
            'EuiseokJeong': {
                'EDA': {'max': 4.007026, 'min': 0},
                'BVP': {'max': 147.035, 'min': -146.605}
            },
            'JoonghooPark': {
                'EDA': {'max': 23.173758, 'min': 0},
                'BVP': {'max': 76.41499999999999, 'min': -74.905},
            },
            'GeesungOh': {
                'EDA': {'max': 5.516068499999999, 'min': 0},
                'BVP': {'max': 101.055, 'min': -102.265},
            },
            'TaesanKim': {
                'EDA': {'max': 6.504631, 'min': 0},
                'BVP': {'max': 77.89999999999999, 'min': -78.57999999999998},
            }
        }

        return tmp_result, min_max_dict
...
def main():
    ...
    dataloader = driving_mode_dataloader(
        ...
        max_km=500, # maximum train data km
        driver='GeesungOh',  # dataset driver(directory name)
        batch_size=16,
        valid_ratio=0.2,  # The number of validation directory
        data='bio',  # ['can', 'front_image', 'side_image', 'bio', 'audio']
        pre_sec=10,  # pre second from self-reporting
    ...
    bio: (batch_size, data_length, 2('EDA', 'BVP'))
    ...
    (batch_size, 4)
        ...
        4 -> [0,0,0,1]
    """
    # train_loader_len = len(list(iter(dataloader.get_train_data)))
    # valid_loader_len = len(list(iter(dataloader.get_valid_data)))
    for epoch in range(epochs):
        # print(f"[INFO] EPOCH {epoch + 1}/{epochs}")
        first_st_time = time.time()
        ...
        st_time = time.time()
        for i, (x, y, serial) in enumerate(train_loader):
        ...
        for i, (x, y, serial) in enumerate(valid_loader):
            print(f"average time: {(time.time() - st_time):.2f}sec")
            st_time = time.time()
            """
            Validataion code
            """
        test_num, data_km, test_list = dataloader.get_test_num()
        for i in range(test_num):
            st_odd = test_list[i]
            odd_km = data_km[i]
            print(f"[INFO] st_odd: {st_odd}, odd_km: {odd_km}")
            test_loader = dataloader.get_test_data(st_odd)
            for j, (x, y, serial) in enumerate(test_loader):
                print(f"average time: {(time.time() - st_time):.2f}sec")
                st_time = time.time()
                """
                Test for (i+1) directory (from 1 to the number of test directory)
                """
        # shuffle train and validation data
        dataloader.shuffle_data()
        print(f"total_time: {time.time() - first_st_time:.2f}sec")
